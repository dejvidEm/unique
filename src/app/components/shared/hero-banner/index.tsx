...
const Herobanner = ({ bannerimage, heading = "", desc = "", headingKey, descKey, headingClass = "large-heading" }: HerobannerProps) => {
    const { t } = useLanguage();
    const displayHeading = headingKey ? t(headingKey) : heading;
    const displayDesc = descKey ? t(descKey) : desc;
    const splitDesc = displayDesc.split(/<\/?span>/);
                ...
                <div className="flex flex-col gap-6  pb-16 xl:pb-20">
                    <div className="flex items-start gap-2 md:gap-6">
                        ...
                        <p className="text-white/70 max-w-md">
                            {splitDesc[0]}
                            {splitDesc[1] != null && <span className="text-primary">{splitDesc[1]}</span>}
                            {splitDesc[2]}
                        </p>
                    </div>
                    <div className="flex flex-row items-end lg:items-baseline gap-4">
                        <h1 className={`${headingClass ? headingClass : "large-heading"}`}>{displayHeading}</h1>
